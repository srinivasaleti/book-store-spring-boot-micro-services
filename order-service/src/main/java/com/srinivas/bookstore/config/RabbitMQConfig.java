...
public class RabbitMQConfig {
    ...
    public Binding bindingCancelled(Queue cancelledOrdersQueue, DirectExchange exchange) {
        return BindingBuilder.bind(cancelledOrdersQueue).to(exchange).with(this.applicationConfig.routingCancel);
    }

    @Bean
    public Binding bindingError(Queue errorOrdersQueue, DirectExchange exchange) {
        return BindingBuilder.bind(errorOrdersQueue).to(exchange).with(this.applicationConfig.routingError);
    }

    @Bean
    public Binding bindingDelivered(Queue deliveredOrdersQueue, DirectExchange exchange) {
        return BindingBuilder.bind(deliveredOrdersQueue).to(exchange).with(this.applicationConfig.routingDelivered);
    }
}
